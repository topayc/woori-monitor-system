...
function gridNumberFormatter(value,row,index){
...
var MONITOR = {
  ...
  initObject : function(){
			  	...
			  	cmenu.menu({
			  		onClick : function(item){
			  			...
			  			case "list_tracking":
			  				...
			  				if (gridData) {
			  					gridData = JSON.parse(context.loadData(item.memberInfo.agentId + "_state_logs"));
			  				}else {
			  					gridData = [];
			  				}
	...
	initAgentCallStatusGrid : function(){
		...
		this.element.agentCallStateGrid.datagrid({
		    ...
		    columns:[[
		        ...
		        {field:'totalCall',title:'전체 상담콜',width:100,align:'center', formatter : gridNumberFormatter},
		        {field:'inCall',title:'응대콜',width:100,align:'center',formatter : gridNumberFormatter},
		        {field:'outCall',title:'발신콜',width:100,align:'center',formatter : gridNumberFormatter},
		        {field:'trnsCall',title:'호전환',width:100,align:'center',formatter : gridNumberFormatter},
		        {field:'cnfrCall',title:'3자통화',width:100,align:'center',formatter : gridNumberFormatter},
		        {field:'ronaCall',title:'No 응대콜',width:100,align:'center',formatter : gridNumberFormatter}
		    ]]
		    //data :  []
		});
	},
		    ...
		    options: {
		      ...
		      onComplete : function(){
		          ...
		          this.data.datasets.forEach(function (dataset) {
		          	for (var i = 0; i < dataset.data.length; i++) {
		          		var model = dataset._meta[Object.keys(dataset._meta)[0]].data[i]._model;
		              ctx.fillText(formatNumber(dataset.data[i]), model.x, model.y);
		            }
		          });
		        }
		      },
		...
		this.agentCallHBarChart = new Chart(ctx_agentCallHBarChart, {
		    ...
		    options: {
		      ...
		      onComplete : function(){
		          ...
		          for (var i = 0; i < dataset.data.length; i++) {
		          	var model = dataset._meta[Object.keys(dataset._meta)[0]].data[i]._model;
		          	ctx.fillText(formatNumber(dataset.data[i]), model.x+5, model.y+7);
		          }
		          });
		            }
		        },
	...
	updateRightTab2 : function(data){
		...
		this.element.totalCallCount.eq(0).text(formatNumber(data.totalSummary.totalCallCount));
  	this.element.totalInCallCount.eq(0).text(formatNumber(data.totalSummary.totalInCallCount));
  	this.element.totalOutCallCount.eq(0).text(formatNumber(data.totalSummary.totalOutCallCount));
  	this.element.totalTrnsCallCount.eq(0).text(formatNumber(data.totalSummary.totalTrnsCallCount));
  	this.element.totalRonaCallCount.eq(0).text(formatNumber(data.totalSummary.totalRonaCallCount));
  	this.element.totalCnfrCallCount.eq(0).text(formatNumber(data.totalSummary.totalCnfrCallCount)); 
  	...
  	this.element.totalCallCount.eq(1).text(formatNumber(bData.totalCallCount));
  	this.element.totalInCallCount.eq(1).text(formatNumber(bData.totalInCallCount));
  	this.element.totalOutCallCount.eq(1).text(formatNumber(bData.totalOutCallCount));
  	this.element.totalTrnsCallCount.eq(1).text(formatNumber(bData.totalTrnsCallCount));
  	this.element.totalRonaCallCount.eq(1).text(formatNumber(bData.totalRonaCallCount));
  	this.element.totalCnfrCallCount.eq(1).text(formatNumber(bData.totalCnfrCallCount)); 
